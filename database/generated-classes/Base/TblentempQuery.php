 ...
 *
 * @method     ChildTblentempQuery leftJoinTblentrcp($relationAlias = null) Adds a LEFT JOIN clause to the query using the Tblentrcp relation
 * @method     ChildTblentempQuery rightJoinTblentrcp($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Tblentrcp relation
 * @method     ChildTblentempQuery innerJoinTblentrcp($relationAlias = null) Adds a INNER JOIN clause to the query using the Tblentrcp relation
 *
 * @method     ChildTblentempQuery joinWithTblentrcp($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the Tblentrcp relation
 *
 * @method     ChildTblentempQuery leftJoinWithTblentrcp() Adds a LEFT JOIN clause and with to the query using the Tblentrcp relation
 * @method     ChildTblentempQuery rightJoinWithTblentrcp() Adds a RIGHT JOIN clause and with to the query using the Tblentrcp relation
 * @method     ChildTblentempQuery innerJoinWithTblentrcp() Adds a INNER JOIN clause and with to the query using the Tblentrcp relation
 *
 * @method     \TblentprsQuery|\TblentclnQuery|\TblentdncQuery|\TblentrcpQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
...
abstract class TblentempQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT idnentemp, uuid, idnentprs, namentemp, logentemp, drcentemp, lclentemp, mncentemp, ententemp, pasentorg, cdgpstemp, cdgtrbemp, girentemp, tlfofiemp, emlofiemp, desaliemp, candonemp, temconemp, horentemp, detentemo, created_at, updated_at, hstentemp FROM tblentemp WHERE idnentemp = :p0';
        try {
    ...
    public function filterByTblentrcp($tblentrcp, $comparison = null)
    {
        if ($tblentrcp instanceof \Tblentrcp) {
            return $this
                ->addUsingAlias(TblentempTableMap::COL_IDNENTEMP, $tblentrcp->getIdnentemp(), $comparison);
