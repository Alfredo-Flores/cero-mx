...
use \FailedJobs as ChildFailedJobs;
use \FailedJobsQuery as ChildFailedJobsQuery;
use \Exception;
use \PDO;
use Map\FailedJobsTableMap;
 ...
 * @method     ChildFailedJobsQuery orderByException($order = Criteria::ASC) Order by the exception column
 ...
 * @method     ChildFailedJobsQuery groupByException() Group by the exception column
 ...
 *
 * @method     ChildFailedJobsQuery leftJoinWith($relation) Adds a LEFT JOIN clause and with to the query
 * @method     ChildFailedJobsQuery rightJoinWith($relation) Adds a RIGHT JOIN clause and with to the query
 * @method     ChildFailedJobsQuery innerJoinWith($relation) Adds a INNER JOIN clause and with to the query
 *
 ...
 *
 * @method     ChildFailedJobs findOneById(string $id) Return the first ChildFailedJobs filtered by the id column
 ...
 * @method     ChildFailedJobs findOneByException(string $exception) Return the first ChildFailedJobs filtered by the exception column
 ...
 *
 * @method     ChildFailedJobs requireOneById(string $id) Return the first ChildFailedJobs filtered by the id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildFailedJobs requireOneByException(string $exception) Return the first ChildFailedJobs filtered by the exception column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildFailedJobs[]|ObjectCollection findById(string $id) Return ChildFailedJobs objects filtered by the id column
 ...
 * @method     ChildFailedJobs[]|ObjectCollection findByException(string $exception) Return ChildFailedJobs objects filtered by the exception column
...
abstract class FailedJobsQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \Base\FailedJobsQuery object.
     *
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = FailedJobsTableMap::getInstanceFromPool(null === $key || is_scalar($key) || is_callable([$key, '__toString']) ? (string) $key : $key)))) {
    ...
    {
        $sql = 'SELECT id, connection, queue, payload, exception, failed_at FROM failed_jobs WHERE id = :p0';
        try {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            FailedJobsTableMap::addInstanceToPool($obj, null === $key || is_scalar($key) || is_callable([$key, '__toString']) ? (string) $key : $key);
        }
        $stmt->closeCursor();
     ...
     * $query->filterByConnection('%fooValue%', Criteria::LIKE); // WHERE connection LIKE '%fooValue%'
     * </code>
     *
     ...
     * $query->filterByQueue('%fooValue%', Criteria::LIKE); // WHERE queue LIKE '%fooValue%'
     * </code>
     *
    ...
    /**
     * Filter the query on the exception column
     *
     * Example usage:
     * <code>
     * $query->filterByException('fooValue');   // WHERE exception = 'fooValue'
     * $query->filterByException('%fooValue%', Criteria::LIKE); // WHERE exception LIKE '%fooValue%'
     * </code>
     *
     * @param     string $exception The value to use as filter.
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return $this|ChildFailedJobsQuery The current query, for fluid interface
     */
    public function filterByException($exception = null, $comparison = null)
    {
        if (null === $comparison) {
        ...
        return $this->addUsingAlias(FailedJobsTableMap::COL_EXCEPTION, $exception, $comparison);
    }
